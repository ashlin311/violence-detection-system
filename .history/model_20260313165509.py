...
from masking import yolo_model, apply_human_mask
...
model.load_state_dict(torch.load("tsn_masked_best.pth", map_location=device))
model.to(device)
model.eval()
...
def run_inference(video_path):
    ...
    for frame in frames:
        masked_frame, count = apply_human_mask(frame, yolo_model)
    ...
    # Class mapping: 0 = Fight, 1 = NonFight
    pred_class = torch.argmax(probs, dim=1).item()
    confidence = probs[0, pred_class].item()
    label = "Violent" if pred_class == 0 else "Non Violent"

    # Most active frame: highest Fight class score (class 0)
    fight_scores = per_frame_logits[0, :, 0]  # [8]
